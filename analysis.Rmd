...
final_storms_data %>%
    group_by(event_type) %>%
    summarise(avg_fatalaties = mean(fatalaties)) %>%
    arrange(desc(avg_fatalaties)) %>%
    head(10) %>%
    ggplot(aes(x = reorder(event_type, avg_fatalaties), y = avg_fatalaties)) +
    geom_bar(stat = "identity") +
    labs(title = "Top 10 most dangerous weather events by fatalaties",
         x = "Event type", y = "Average number of fatalaties") +
    coord_flip()
```
...
is at the top, but this time followed by **hurricane/typhoon**, **heat waves**
and others.
