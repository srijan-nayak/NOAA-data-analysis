...
for (evtype in valid_evtpyes) {
    ...
    is_evtype <- grepl(generate_regex(evtype), raw_storms_data$EVTYPE,
                       ignore.case = TRUE)
    # change the matched event types to the valid event type
    raw_storms_data <- raw_storms_data %>%
...
filtered_storms_dataset <- raw_storms_data %>%
    filter(EVTYPE %in% valid_evtpyes)

raw_length <- dim(raw_storms_data)[1]
