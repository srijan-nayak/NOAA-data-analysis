...
```{r select_variables}
selected_storms_dataset <- raw_storms_data %>% select(
    EVTYPE,
    FATALITIES,
    INJURIES,
    PROPDMG,
...
for (evtype in valid_evtpyes) {
    ...
    is_evtype <- grepl(generate_regex(evtype), selected_storms_dataset$EVTYPE,
                       ignore.case = TRUE)
    # change the matched event types to the valid event type
    selected_storms_dataset <- selected_storms_dataset %>%
...
filtered_storms_dataset <- selected_storms_dataset %>%
    filter(EVTYPE %in% valid_evtpyes)

raw_length <- dim(selected_storms_dataset)[1]
filtered_length <- dim(filtered_storms_dataset)[1]
```
